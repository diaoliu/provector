...
<Callout>
  Press <kbd>backspace</kbd> or <kbd>delete</kbd> to remove the points and curve
  segments that connect these points.
</Callout>
...
To activate the direct selection tool, you can edither
...
In the anchors panel, you will see following commands:

- **Convert to Corner Points**: this command clears the handles of the selected points.
- **Convert to Smooth Points**: this command adds two collinear handles to form a smooth point.
- **Cut Paths**: this command splits paths at the selected points.
- **Join Paths**: this command joins two open paths at the selected endpoints.
- **Remove Anchor Points**: Unlike deleting points from the paths, this command removes also the curve segments the points belong to.
...
2. Select Menubar > Object > Path > Offset Paths.
...
1. Select one or more paths.
2. Select Menubar > Object > Path > Smooth Paths.
...
2. Select Menubar > Object > Path > Blend Paths.
...
1. Select one or more paths.
2. Select Menubar > Object > Path > Simplify Paths.
...
Select Menubar > Object > Path > Outline Stroke, this command will convert the stroke of a path into a separate path that you can edit independently.
